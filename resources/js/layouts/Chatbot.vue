  <template>
    <div>
      ...
      <div 
        v-show="isOpen"
        class="fixed bottom-20 right-4 w-[90vw] sm:w-[380px] max-w-full bg-white border border-gray-200 rounded-2xl shadow-2xl z-[9998] max-h-[65vh] sm:max-h-[500px]"
      >
        <!-- Header - Smaller -->
        ...
        <div 
          ...
          class="h-32 sm:h-40 overflow-y-auto p-3 space-y-3 bg-gray-50"
        >
          <div 
  ...
  const getGeminiResponse = async (userMessage, context = '') => {
    ...
    const GEMINI_API_KEY = 'AIzaSyB_c_BslWVoCXhulDQQ4nIe6_uKygArrIA'; // Replace with your actual key
  ...
  const faqDatabase = {
      ...
      },
    {
        ...
        answer: 'We are open Monday to Friday from 8:00 AM to 5:00 PM (PST). We are closed on weekends and public holidays. For urgent matters outside business hours, please contact our emergency hotline at +63(2) 852989785.',
        keywords: ['hours', 'open', 'close', 'business hours', 'time', 'schedule', 'operating hours', 'when open']
      },
      {
        question: 'Do you have parking space available?',
        answer: 'Yes, we provide ample parking space for visitors and customers. Our facility at 016 Panapaan 2, Bacoor City has a dedicated parking area that can accommodate cars, vans, and small trucks. Parking is free for all visitors. Please check in at the security gate upon arrival.',
        keywords: ['parking', 'park', 'space', 'lot', 'car', 'vehicle', 'where to park']
      }
    ],
  };
  ...
  const getGreetingResponse = () => {
    const responses = [
      "Hello! 👋 Welcome to JPMC. How can I assist you today?",
      "Hi there! 😊 I'm here to help with any questions about JPMC's services.",
      "Hey! Great to hear from you. What can I help you with?",
      "Hello! Welcome to James Polymers Manufacturing Corporation. How may I help you?",
      "Hi! 🙂 I'm your JPMC assistant. Feel free to ask me anything about our services!",
      "Greetings! How can I assist you with JPMC's manufacturing solutions today?"
    ];
  ...
  const sendMessage = async () => {
    ...
    if (isGreeting(message)) {
      setTimeout(() => {
        ...
        setTimeout(() => {
          addBotMessage("What would you like to know about? Choose a category:");
          showMainCategories();
        }, 1000);
      }, 500);
      return;
